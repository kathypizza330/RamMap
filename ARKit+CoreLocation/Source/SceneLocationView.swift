...
public class SceneLocationView: ARSCNView, ARSCNViewDelegate {
    ...
    // dictionary for different filters
    public var locationDictionary = [String: [LocationNode]]()
    ...
    public func getDicKeys() -> [String]{
        var keys = [String]()
        for dic in locationDictionary{
            keys.append(dic.0)
        }
        return keys
    }
    
    // change the nodelist according to key
    public func changeNodes(keys: [String]){
        ...
        for key in keys{
            for node in locationDictionary[key]! {
    ...
    // add node set to the dictionary
    public func addSetToDic(setName: String, nodeSet: [LocationNode]){
        locationDictionary[setName] = nodeSet
    }
    
    // add all node in the dic to node list
    public func addAllNodesFromDic(){
    ...
    public func updatePositionAndScaleOfLocationNode(locationNode: LocationNode, initialSetup: Bool = false, animated: Bool = false, duration: TimeInterval = 0.1) {
        ...
        if let annotationNode = locationNode as? LocationAnnotationNode {
            ...
            // text child node
            //            let textNode = locationNode.childNode(withName: "child", recursively: true)
